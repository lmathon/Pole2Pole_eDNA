...
data <- exp_var
df <- data %>%
  dplyr::select(-c("station", "province")) # remove station
...
dbrda_MPD_province <- ggplot(station_scores_met, aes(x= CAP1, y = CAP2)) +
  ...
  geom_point(aes(col = province), cex = 2, show.legend = T) +
...
save(dbrda_MPD_province, file="Rdata/dbrda_MPD_province.rdata")
