...
FD_rich_station$province[is.na(FD_rich_station$province)] <- "Tropical_Southwestern_Pacific"
...
SR_FDq0 <- ggplot(FD_rich_station, aes(x=MOTUs, y=FD_q0)) +
  ...
  labs(x=expression(paste("MOTU ", alpha,"-diversity")), y= expression(paste("Sequence ", alpha,"-diversity")))+
...
ggsave("outputs/Figures_papier/Fig1.pdf", width = 7.5, height = 7.2, dpi = 600)
ggsave("outputs/Figures_papier/Fig1.png", width = 7.5, height = 7.2, dpi = 600)

cor.test(FD_rich_station$FD_q0, FD_rich_station$MOTUs, method = "spearman")
