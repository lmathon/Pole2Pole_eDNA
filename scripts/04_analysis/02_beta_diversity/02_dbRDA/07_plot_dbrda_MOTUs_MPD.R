...
load("Rdata/dbrda_MPD_province.rdata")
...
dbrda_province <- ggarrange(dbrda_MOTUs_province, dbrda_MPD_province, 
                            common.legend = T, legend = c("bottom"), 
                            labels = c("", "D. MPD"), font.label = list(size = 12, color = "black"),
                            label.x = 0)

dbrda_variable <- ggarrange(dbrda_MOTUs_SST, dbrda_MPD_SST, dbrda_MOTUs_MED, dbrda_MPD_MED, nrow=2, ncol=2, 
          labels = c("B", "E", "C", "F"), 
          font.label = list(size = 12, color = "black"),
          label.x = 0)
ggarrange(dbrda_province, dbrda_variable, nrow=2, ncol=1, heights = c(1,2))

ggsave("outputs/dbRDA/dbrda_jaccard_MPD.png", width = 10, height = 12)
