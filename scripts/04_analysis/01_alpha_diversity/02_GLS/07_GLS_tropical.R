...
# select tropical regions

data <- data %>%
  filter(province %in% c("Western_Indian_Ocean", "Western_Coral_Triangle", "Tropical_Northwestern_Atlantic", "Southeast_Polynesia", "Tropical_East_Pacific", "Tropical_Southwestern_Pacific"))
...
#### GLS largefish ####
gls.large <- gls(largefish_MOTUs ~ mean_DHW_1year+mean_DHW_5year+mean_sss_1year+mean_SST_1year+mean_npp_1year+Corruption_mean+HDI2019+Gravity+MarineEcosystemDependency+conflicts+dist_to_CT+bathy+depth_sampling+distCoast+volume, correlation = corGaus(form = ~longitude_start + latitude_start, nugget = TRUE), data = data,method="ML")

large_effectsize <- effectsize(gls.large)
...
save(motus_effectsize, file = "Rdata/effectsize_tropical.rdata")
...
ggsave("outputs/GLS/effect_size_tropical.png", width = 10, height = 6.5)
